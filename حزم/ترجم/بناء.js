...
function replaceIndents(s) {
  return s.replace(/^( {2})+/gm, (m) => new Array(m.length / 2 + 1).fill('').join(indentPH));
}

let translatorCodeStr = JSON.stringify(
  replaceIndents(translatorContent.replace('exports.default = ', ''))
);
let requireCodeStr = JSON.stringify(
  replaceIndents(requireContent.replace('exports.default = ', ''))
);
